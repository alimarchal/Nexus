...
use App\Http\Controllers\AuditFindingAttachmentController;
use App\Http\Controllers\AuditMetricsCacheController;
use App\Http\Controllers\AuditNotificationController;
use App\Http\Controllers\AuditScheduleController;
use App\Http\Controllers\AuditTagController;
use App\Http\Controllers\AuditTagPivotController;
...
Route::middleware(['auth:sanctum', config('jetstream.auth_session'), 'verified'])->group(function () {
    ...
    Route::prefix('products/audits/{audit}')->name('audits.')->group(function () {
        ...
        Route::get('full', [AuditController::class, 'fullData'])->name('full');
        // Basic Audit Updates
    ...
    Route::prefix('audit-management')->name('audit-management.')->group(function () {
        // Audit Types Management
        Route::resource('types', AuditTypeController::class)->names('audit-types');

        // Global Audit Tags Management
        Route::resource('tags', AuditTagController::class)->names('audit-tags');

        // Audit Tag Pivots (for assigning tags to audits)
        Route::post('audits/{audit}/tags', [AuditTagPivotController::class, 'store'])->name('audit-tag-pivots.store');
        Route::delete('audits/{audit}/tags/{tag}', [AuditTagPivotController::class, 'destroy'])->name('audit-tag-pivots.destroy');

        // Audit Schedules Management
        Route::post('audits/{audit}/schedules', [AuditScheduleController::class, 'store'])->name('audit-schedules.store');
        Route::patch('audits/{audit}/schedules/{schedule}', [AuditScheduleController::class, 'update'])->name('audit-schedules.update');
        Route::delete('audits/{audit}/schedules/{schedule}', [AuditScheduleController::class, 'destroy'])->name('audit-schedules.destroy');

        // Audit Notifications Management
        Route::post('audits/{audit}/notifications', [AuditNotificationController::class, 'store'])->name('audit-notifications.store');
        Route::patch('audits/{audit}/notifications/{notification}', [AuditNotificationController::class, 'update'])->name('audit-notifications.update');
        Route::delete('audits/{audit}/notifications/{notification}', [AuditNotificationController::class, 'destroy'])->name('audit-notifications.destroy');
        Route::post('audits/{audit}/notifications/{notification}/resend', [AuditNotificationController::class, 'resend'])->name('audit-notifications.resend');

        // Audit Action Updates Management
        Route::post('audits/{audit}/actions/{action}/updates', [AuditActionUpdateController::class, 'store'])->name('audit-action-updates.store');
        Route::patch('audits/{audit}/actions/{action}/updates/{update}', [AuditActionUpdateController::class, 'update'])->name('audit-action-updates.update');
        Route::delete('audits/{audit}/actions/{action}/updates/{update}', [AuditActionUpdateController::class, 'destroy'])->name('audit-action-updates.destroy');

        // Audit Finding Attachments Management
        Route::post('audits/{audit}/findings/{finding}/attachments', [AuditFindingAttachmentController::class, 'store'])->name('audit-finding-attachments.store');
        Route::delete('audits/{audit}/findings/{finding}/attachments/{attachment}', [AuditFindingAttachmentController::class, 'destroy'])->name('audit-finding-attachments.destroy');
        Route::get('audits/{audit}/findings/{finding}/attachments/{attachment}/download', [AuditFindingAttachmentController::class, 'download'])->name('audit-finding-attachments.download');

        // Audit Metrics Cache Management
        Route::post('audits/{audit}/metrics-cache/recalculate', [AuditMetricsCacheController::class, 'recalculate'])->name('audit-metrics-cache.recalculate');
        Route::delete('audits/{audit}/metrics-cache', [AuditMetricsCacheController::class, 'clear'])->name('audit-metrics-cache.clear');
    });

    // Add route aliases for backward compatibility
    Route::name('audit-types.')->group(function () {
        Route::post('audit-types', [AuditTypeController::class, 'store'])->name('store');
        Route::delete('audit-types/{type}', [AuditTypeController::class, 'destroy'])->name('destroy');
    });

    Route::name('audit-tags.')->group(function () {
        Route::post('audit-tags', [AuditTagController::class, 'store'])->name('store');
        Route::delete('audit-tags/{tag}', [AuditTagController::class, 'destroy'])->name('destroy');
    });

    Route::name('audit-tag-pivots.')->group(function () {
        Route::post('audits/{audit}/tag-pivots', [AuditTagPivotController::class, 'store'])->name('store');
        Route::delete('audits/{audit}/tag-pivots/{tag}', [AuditTagPivotController::class, 'destroy'])->name('destroy');
    });
