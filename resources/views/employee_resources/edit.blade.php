<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight inline-block">
            Edit Employee Resource: {{ $employeeResource->resource_number }}
        </h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                ...
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('employee_resources.update', $employeeResource) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                            <div class="flex flex-col md:flex-row gap-4">
                                <div class="flex-1">
                                    ...
                                    <x-input id="resource_no" type="text" name="resource_no"
                                        class="mt-1 block w-full" :value="old('resource_no', $employeeResource->resource_no)" required />
                                </div>
        ...
        @endforeach
                                    ...
                                    <select id="division_id" name="division_id"
                                        ...
                                        @foreach ($divisions as $division)
                                            <option value="{{ $division->id }}"
                                                {{ old('division_id', $employeeResource->division_id) == $division->id ? 'selected' : '' }}>
                                                {{ $division->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select id="category_id" name="category_id"
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                {{ old('category_id', $employeeResource->category_id) == $category->id ? 'selected' : '' }}>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea id="description" name="description"
                                ...
                                rows="3">{{ old('description', $employeeResource->description) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            @if ($employeeResource->attachment)
                                <div class="mb-2">
                                    ...
                                    <a href="{{ Storage::url($employeeResource->attachment) }}"
