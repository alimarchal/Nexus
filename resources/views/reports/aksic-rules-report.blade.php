        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg print:shadow-none">
            ...
            <div class="p-4 print:p-0">
                ...
                <table class="w-full text-sm border-collapse border border-slate-400 text-left text-black dark:text-gray-300 print:text-black">
                    <caption class="caption-top text-left p-2 font-bold">District-wise AKSIC Rules and Loan Position</caption>
                    <thead class="text-black uppercase bg-gray-50 dark:bg-gray-700 print:bg-gray-200">
                        <tr>
                            ...
                            <th class="px-2 py-2 border border-black text-right">Interest</th>
                            <th class="px-2 py-2 border border-black text-right">Total Payable</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($reportRows as $row)
                            <tr class="{{ $loop->even ? 'bg-gray-50 dark:bg-gray-700 print:bg-gray-100' : '' }}">
                                <td class="px-2 py-2 border border-black text-center">{{ $loop->iteration }}</td>
                                ...
                                <td class="px-2 py-2 border border-black text-right">{{ number_format($row['interest_amount'], 2) }}</td>
                                <td class="px-2 py-2 border border-black text-right">{{ number_format($row['total_payable'], 2) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot class="font-bold bg-gray-100 dark:bg-gray-700 print:bg-gray-200">
                        <tr>
                            ...
                            <td class="px-2 py-2 border border-black text-right">{{ number_format($totals['interest_amount'], 2) }}</td>
                            <td class="px-2 py-2 border border-black text-right">{{ number_format($totals['total_payable'], 2) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
