<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <div class="flex justify-center items-center float-right">
            ...
            <a href="{{ route('daily-positions.create') }}"
                ...
                <form method="GET" action="{{ route('daily-positions.index') }}">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="date_range" name="filter[date_range]"
                                class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full"
                                value="{{ request('filter.date_range') }}" />
                        </div>
                        <div></div>
                        <div class="mt-4">
                        ...
                        <tbody class="text-black text-md leading-normal font-medium">
                            @foreach ($dailyPositions as $position)
                                <tr class="border-b border-gray-200 hover:bg-gray-100">
                                    ...
                                    <td class="py-1 px-2 text-center">{{ number_format($position->totalAssets, 3) }}
                                    </td>
                                    <td class="py-1 px-2 text-center">{{ number_format($position->totalDeposits, 3) }}
                                    </td>
                                    <td class="py-1 px-2 text-center">{{ number_format($position->totalCasaTdr, 3) }}
                                    </td>
                                    ...
                                    <td class="py-1 px-2 text-center">
                                        <a href="{{ route('daily-positions.view', $position) }}"
                                        ...
                                        <a href="{{ route('daily-positions.edit', $position) }}"
                                            class="inline-flex items-center px-4 py-2 bg-green-800 text-white rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
                                            Edit
                                        </a>
                                        <form class="inline-block" method="POST"
                                            action="{{ route('daily-positions.destroy', $position) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
        ...
        <script type="text/javascript">
            $(function() {
                ...
                $('#date_range').on('apply.daterangepicker', function(ev, picker) {
                    $(this).val(picker.startDate.format('MM/DD/YYYY') + ' to ' + picker.endDate.format(
                        'MM/DD/YYYY'));
                });
