<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex justify-center items-center float-right print:hidden">
            ...
            @endcan
            @can('delete aksics')
                <form method="POST" action="{{ route('aksic.destroy', $aksic) }}" class="inline-block" onsubmit="return confirm('Delete this AKSIC record?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl print:shadow-none">
            ...
            <div class="p-4 print:p-0">
                <div class="text-center mb-4">
                    <h1 class="print-title text-xl font-bold text-gray-900 dark:text-gray-100">AKSIC Amortization Schedule</h1>
                ...
                <table class="mb-4 w-full text-sm border-collapse border border-slate-400 text-left text-black dark:text-gray-300 print:text-black">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-gray-50 dark:bg-gray-700 print:bg-gray-200">
                            ...
                            <td class="px-2 py-2 border border-black">{{ $aksic->businessSubCategory?->name ?? '-' }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr class="bg-gray-50 dark:bg-gray-700 print:bg-gray-200">
                            ...
                            <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $aksic->principal_amount, 2) }}</td>
                        ...
                        <tr>
                            ...
                            <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $aksic->kibor_rate, 2) }}%</td>
                            <th class="px-2 py-2 border border-black">Spread</th>
                            <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $aksic->spread_rate, 2) }}%</td>
                        </tr>
                        <tr class="bg-gray-50 dark:bg-gray-700 print:bg-gray-200">
                            <th class="px-2 py-2 border border-black">Total Rate</th>
                            <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $aksic->total_rate, 2) }}%</td>
                            <th class="px-2 py-2 border border-black">Disbursement Date</th>
                            <td class="px-2 py-2 border border-black">{{ $aksic->disbursement_date?->format('d-M-Y') }}</td>
                        </tr>
                        <tr>
                ...
                <table class="w-full text-sm border-collapse border border-slate-400 text-left text-black dark:text-gray-300 print:text-black">
                    ...
                    <thead class="text-black uppercase bg-gray-50 dark:bg-gray-700 print:bg-gray-200">
                        <tr>
                            ...
                            <th class="px-2 py-2 border border-black text-center">Date</th>
                    ...
                    <tbody>
                        @foreach ($aksic->amortizations as $row)
                            <tr class="{{ $loop->even ? 'bg-gray-50 dark:bg-gray-700 print:bg-gray-100' : '' }}">
                                ...
                                <td class="px-2 py-2 border border-black text-center">{{ $row->due_date->format('d-M-y') }}</td>
                                <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $row->principal_amount_os, 2) }}</td>
                                <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $row->installment_per_month, 2) }}</td>
                                <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $row->product, 2) }}</td>
                                <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $row->interest_rate_per_month, 2) }}</td>
                                <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $row->total_interest, 2) }}</td>
                                <td class="px-2 py-2 border border-black text-center">{{ $row->days }}</td>
                                <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $row->total_installment, 2) }}</td>
                                <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $row->principal_balance_after_installment, 2) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot class="font-bold bg-gray-100 dark:bg-gray-700 print:bg-gray-200">
                        <tr>
                            ...
                            <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $aksic->amortizations->sum('total_interest'), 2) }}</td>
                            <td class="px-2 py-2 border border-black text-center">{{ $aksic->amortizations->sum('days') }}</td>
                            <td class="px-2 py-2 border border-black text-right">{{ number_format((float) $aksic->amortizations->sum('total_installment'), 2) }}</td>
