        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden border border-gray-200">
                ...
                <div id="scopes-tab" class="tab-content p-6" style="display:none;">
                    <div class="space-y-4">
                        @forelse($audit->scopes as $scope)
                        <div class="p-3 border rounded bg-white flex justify-between">
                            <div>
                                <div class="font-medium text-gray-800">{{ $scope->scope_item }}</div>
                                <div class="text-xs text-gray-500">{{ $scope->is_in_scope ? 'In Scope' : 'Out of Scope'
                                    }} @if($scope->description) • {{ Str::limit($scope->description,80) }} @endif</div>
                            </div>
                            <form method="POST" action="{{ route('audits.scopes.delete', [$audit, $scope]) }}">@csrf
                                @method('DELETE')<button class="text-xs px-2 py-1 bg-red-600 text-white rounded"
                                    onclick="return confirm('Delete scope?')">Delete</button></form>
                        </div>
                        @empty
                        <p class="text-sm text-gray-500">No scope items.</p>
                        @endforelse
                        <div class="mt-4 p-4 bg-gray-50 border rounded">
                            <h5 class="text-sm font-semibold mb-2">Add Scope Item</h5>
                            <form method="POST" action="{{ route('audits.scopes.add', $audit) }}" class="space-y-2">
                                @csrf
                                <input name="scope_item" required placeholder="Scope item"
                                    class="w-full border-gray-300 rounded-md text-sm">
                                <textarea name="description" rows="2" class="w-full border-gray-300 rounded-md text-sm"
                                    placeholder="Description (optional)"></textarea>
                                <label class="inline-flex items-center text-xs"><input type="checkbox"
                                        name="is_in_scope" value="1" checked
                                        class="rounded border-gray-300 text-indigo-600"><span class="ml-2">In
                                        Scope</span></label>
