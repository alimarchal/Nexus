                ...
                <form method="POST" action="{{ route('branches.update', $branch) }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="district_id" class="select2 mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                @foreach ($districts as $district)
                                    <option value="{{ $district->id }}" {{ $district->id == $branch->district_id ? 'selected' : '' }}>
                                        {{ $district->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-input id="code" type="text" name="code" class="mt-1 block w-full" :value="old('code', $branch->code)" required />
                        </div>

                        <div>
                            <x-label for="name" value="Name" :required="true" />
                            <x-input id="name" type="text" name="name" class="mt-1 block w-full" :value="old('name', $branch->name)" required />
                        </div>

                        <div>
                            <x-label for="address" value="Address" :required="true" />
                            <x-input id="address" type="text" name="address" class="mt-1 block w-full" :value="old('address', $branch->address)" required />
                        </div>
                    </div>
